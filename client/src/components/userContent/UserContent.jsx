...
const UserContent = ({toggleMenu}) => {
  ...
  return (
      <div className='hidden md:flex w-[100%] md:w-[30%] flex-col gap-2 '>
        <div className='flex md:flex-col justify-between h-fit w-full bg-white rounded-md p-4 border-[1px] border-gray-300'>
          ...
          <div className='hidden md:block'>
            ...
            <div >
              ...
              <p className='text-gray-700'>{userInfo?.year !== 0 ? userInfo?.year + " year" : ""}</p>
            </div>
            <p className={`${userInfo?.city !== '' ? "block" : "hidden"}`}>Resident of: {userInfo?.city}</p>

          </div>
          <img onClick={toggleMenu} className='block md:hidden w-[30px] cursor-pointer' src={menuBar} alt="" />
        </div>
        <div className='hidden md:block h-fit w-full bg-white rounded-md p-4 border-[1px] border-gray-300'>
          {userInfo?.posts.length > 0 ? <p>Total notes uploaded: {userInfo?.posts.length}</p> : <p>No notes uploaded yet.</p>}
          <p>{userInfo?.rating === 0 ? "No rating yet." : `Rating: ${userInfo?.rating}`}</p>
        </div>
        <div className='hidden md:flex h-fit w-full bg-white rounded-md p-4 flex-col gap-4 border-[1px] border-gray-300'>
