...
const Bio = () => {
  ...
  return (
    <div className={`relative bg-white p-4 lg:w-[100%] h-fit rounded-md w-[100%]  border-[1px] border-gray-300`}>
        <div onClick={toggleExpanded} className={`${!isExpanded ? 'line-clamp-3' : ''}`}>
            ...
            {userInfo?.bio ? <p>{userInfo?.bio}</p> : <Link to="/update-profile" state={{ section: 'Bio' }}>Add bio</Link>}
        </div>
